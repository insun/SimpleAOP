...
while($i++ < $opts['repeat']) {
    ...
    defined('AOP_ALREADY_INIT') || define('AOP_ALREADY_INIT', true);
}

$time = microtime(true) - $time;
$memory = memory_get_peak_usage();

echo str_pad('average with AOP : ', 15) . str_pad(round($time*1000/$i, 3), 5, '0') . ' ms, ' . str_pad(round($memory/1024/1024, 2), 2, '0') . 'MB' . "\n";
